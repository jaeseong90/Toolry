      ...
      <div className="max-w-4xl mx-auto space-y-4">
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="text-sm text-gray-400">한글 단어 입력</label>
            <button
          ...
          <input
            ...
            placeholder="예: 사용자 이름, 주문번호, 결제금액, 배송 상태 ..."
            className="w-full"
          />
          <p className="text-xs text-gray-600 mt-1">
            띄어쓰기로 단어를 조합할 수 있습니다 (예: &ldquo;주문 상태&rdquo; → order_status)
          </p>
        </div>
        ...
        {results.length > 0 && (
          <div className="space-y-3">
            {results.map((suggestion, idx) => (
              <div key={idx} className="bg-surface border border-gray-700 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-sm font-medium text-white">
                    {suggestion.entry.ko[0]}
                  </span>
                  ...
                  {suggestion.entry.ko.length > 1 && (
                    <span className="text-xs text-gray-600">
                      ({suggestion.entry.ko.slice(1).join(", ")})
                    </span>
                  )}
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {suggestion.formats.map((fmt) => (
                    <div key={fmt.label} className="flex items-center justify-between bg-bg rounded-md px-3 py-2">
                      <div>
                        <div className="text-[10px] text-gray-500 mb-0.5">{fmt.label}</div>
                        <div className="font-mono text-sm text-gray-200">{fmt.value}</div>
                      </div>
        ...
        {showDict && (
          <div className="border border-gray-700 rounded-lg overflow-hidden">
            <div className="bg-surface px-4 py-3 border-b border-gray-700">
              <h3 className="text-sm font-medium text-white">
                지원 단어 목록 ({dictionary.length}개)
              </h3>
            </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {entries.map((entry, i) => (
                      <button
                        ...
                        className="px-2 py-1 text-xs bg-bg border border-gray-700 rounded text-gray-400 hover:text-accent hover:border-accent/50 transition-colors cursor-pointer"
                      >
                        {entry.ko[0]} → {entry.en}
                      </button>
                    ))}
